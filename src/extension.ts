...
import { create as createReport } from './generators/report';
...
const isUpperCamel = thing => (
    thing === ASSET || thing === REPORT || thing === VIEW || thing === COMPONENT
);
...
const getTypeOfThing = () => {
    ...
    return window.showQuickPick(createOptions)
        .then(type => type)
}

const getNameOfThing: (type: string, path?: string) => Thenable<Thing | undefined> = (type: string, path?: string) => {
    if (type) {
...
const createThing = (thing: Thing) => {
    ...
    switch(thing.type) {
        ...
        case REPORT: {
            fileData = createReport(thing.name);
            break;
        }
        case SERVICE: {
...
export function activate(context: ExtensionContext) {
    ...
    const generateComponent = commands.registerCommand('extension.generateComponent', (context) => {
        if (context && context.fsPath) {
            const projectRoot = workspace.workspaceFolders[0].uri.fsPath.split('\\').join('/');
            const path = `${context.fsPath}/`.split('\\').join('/').replace(projectRoot, '');
